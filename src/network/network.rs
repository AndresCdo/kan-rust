...
impl Network {
    ...
    pub fn backward(&self, input: Vector, target: Vector) -> (Matrix, Vector) {
        ...
        for i in (1..self.layers.len()).rev() {
            ...
            let gradient =
                output.elementwise_multiply(&output.subtract(&Vector::ones(output.len())));
        ...
        let mut total_weight_gradients = Matrix::zeros(
            self.layers[0].weights.col_count(),
            self.layers[0].weights.row_count(),
        );
    ...
    pub fn update(&mut self, weight_gradients: &Matrix, delta: &Vector, learning_rate: f32) {
        self.layers[0].weights = weight_gradients
            .scalar_multiply(learning_rate)
            .add(&self.layers[0].weights)
            .unwrap();
        self.layers[0].biases = delta
            .scalar_multiply(learning_rate)
            .add(&self.layers[0].biases);
    }
    ...
    pub fn predict_batch(&self, inputs: &[Vector]) -> Vec<Vector> {
        inputs
            .iter()
            .map(|input| self.predict(input.clone()))
            .collect()
    }
    ...
    pub fn loss_batch(&self, inputs: &[Vector], targets: &[Vector]) -> f32 {
        let total_loss: f32 = inputs
            .iter()
            .zip(targets)
            .map(|(input, target)| self.loss(input.clone(), target.clone()))
            .sum();
        total_loss / inputs.len() as f32
    }
    ...
    pub fn accuracy_batch(&self, inputs: &[Vector], targets: &[Vector]) -> f32 {
        let total_accuracy: f32 = inputs
            .iter()
            .zip(targets)
            .map(|(input, target)| self.accuracy(input.clone(), target.clone()))
            .sum();
        total_accuracy / inputs.len() as f32
    }
